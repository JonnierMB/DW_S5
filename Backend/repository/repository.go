...
//En el repositorio estarán las operaciones sobre la base de datos
import (
	"context"
	"database/sql"
	"fmt"
...
type Repository[Entity any] interface { //DEFINICIÓN DE METODOS DE LA INTERFAZ
...
type repository[Entity any] struct {
	connection *sqlx.DB //CONEXIÓN A BASE DE DATOS SQLX ELEPHANT ETC
}

// Retrieves a Persistence object customized for the specific data types
func NewRepository[Entity any](conn *sqlx.DB) (*repository[Entity], error) { // [REPOSITORIO GENERICO ENTITY ANY]
	if conn == nil {
